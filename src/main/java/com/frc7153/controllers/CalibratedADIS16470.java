...
public class CalibratedADIS16470 {
    ...
    public class CalibrateIMU extends CommandBase {
        ...
        public void end(boolean terminated) {
            if (!terminated) { calibrated = true; }
        }

        // Is done (4 seconds)
        @Override
        public boolean isFinished() { return (Timer.getFPGATimestamp() - start) >= 4.0; }
    }
     ...
     */
    public double getAccelY() { warnIfNotCalibrated(); return imu.getAccelY() - offsets[0]; }

    /**
     * @return Current z acceleration of the gyro
     */
    public double getAccelZ() { warnIfNotCalibrated(); return imu.getAccelZ() - offsets[0]; }

    /**
     * @return Yaw angle in degrees, CCW positive
     */
    public double getYaw() { warnIfNotCalibrated(); return imu.getAngle(); }

    /**
     * @return Roll angle in degrees, CCW positive
     */
    public double getRoll() { warnIfNotCalibrated(); return imu.getXComplementaryAngle(); }

    /**
     * @return Pitch angle in degrees, CCW positive
     */
    public double getPitch() { warnIfNotCalibrated(); return imu.getYComplementaryAngle(); }
}
