...
public abstract class ValidatedObject {
    /**
     * Validates the object by checking everything. The maps returned should be the same size,<br>
    ...
    /**
     * Assign this validated object to a Shuffleboard Layout. Each object can only be on ONE layout.
    ...
    public void assignShuffleboardValidationLayout(ShuffleboardLayout layout) {
        if (shuffleboardLayout != null) {
            DriverStation.reportWarning("Overwrote shuffleboard layout for validated object!", false);
